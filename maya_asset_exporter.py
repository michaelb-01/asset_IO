...
def getLatestVersion(exportPath):
...
def exportAssetJsonFile(assetPath, assetName, assetVersion, dryRun=None):
...
def onAssetNameChange(assetName):
...
def getExportDir():
    ...
    if not os.path.exists(exportDir):
        if publishing:
            mc.warning('Could not find a _publish3d scene above the maya folder')
        else:
            mc.warning('Could not find an export scene above the maya folder')
        return False
    else:
        return exportDir
